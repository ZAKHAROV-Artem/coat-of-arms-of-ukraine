...
// Static data. Array of image attributes to iterate through and render Next.js Image component.
const IMAGES = [
  {
    id: 1,
...
// Static data. Can be useful for translation.
const TITLE = "Український тризуб";
const SUBTITLE = "Coat of arms of Ukraine";

// Long SVG code. Probably better to use it like this for clean code in JSX.
const SvgIconWithAnimation = () => (
  <svg
...
// Maybe move it to a separate file to correctly test
const usePlayAnimation = ({
  scope,
  onFinish,
}: {
  ...
  // Phase 1 => show intro, start phase 2 when finish
  useGSAP(
    () => {
      gsap
        .timeline()
  ...
  // Phase 2 => run animation, call "onFinish" callback
  useGSAP(
    () => {
    ...
    { dependencies: [currentPhase], scope }
  );
};
...
export default function Intro() {
  ...
  // Clean hook with good name. 8 lines of code in our compoment and we can easy guess what it does. Right?
  usePlayAnimation({
    scope: introRef,
    onFinish: () => {
      setTimeout(() => {
  ...
  // Only one variable with good name. It is enough to hide "Intro" after animation is done. No need to change opacity or some other stuff.
  if (isHidden) return null;
  
  return (
    <div
      ref={introRef}
      className="fixed top-0 z-50 h-screen w-full bg-bg duration-500"
    >
